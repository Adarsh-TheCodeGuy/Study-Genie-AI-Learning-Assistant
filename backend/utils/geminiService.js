...
if (!process.env.GEMINI_API_KEY) {
  console.error(
    "FATAL ERROR: GEMINI_API_KEY is not set in the eniroment variables.",
  );
  process.exit(1);
}
...
export const generateQuiz = async (text, numQuestions = 5) => {
  const prompt = `Generate exactly ${numQuestions} multiple choice questions from the following text.
    ...
    O3: [Option 3]
    O4: [Option 4]
    C: [Write ONLY the full correct answer text. Do NOT include O1, O2, O3, or O4.]
    E: [Brief explanation]
    ...
    for (const block of questionBlocks) {
      ...
      for (const line of lines) {
        ...
        if (trimmed.startsWith("Q:")) {
          ...
          // Remove O1:, O2:, etc. if Gemini includes it anyway
...
export const generateSummary = async (text) => {
  const prompt = `Provide a concise summary of the following text, highlighting the key concepts, main ideas, an important points.
...
export const chatWithContext = async (question, chunks) => {
  ...
  try {
    ...
    console.error("Gemini API error:", error);
    throw new Error("Failed to process chat resquest");
  }
};
